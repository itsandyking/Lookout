...
def compute_iteration_result(
    ...
    scores = [qs for _, _, qs in results]
    ...
    # Deterministic total (exclude fidelity)
    det_totals = []
    for s in scores:
        det = sum(s.axes[name].score for name in axis_names if name in s.axes)
        det_totals.append(det)

    avg_score = sum(det_totals) / len(det_totals) if det_totals else 0
    max_score = max(det_totals) if det_totals else 0
...
async def run_optimization_loop(
    ...
    baseline = compute_iteration_result(0, results, str(prompt_path))
    ...
    for i in range(1, max_iterations + 1):
        ...
        suggestion = await _suggest_improvement(llm, current_prompt, history)
...
async def _suggest_improvement(
    ...
    """Use the LLM to suggest a prompt improvement based on scoring data."""
    latest = history[-1]
...
Weakest products:
{chr(10).join(f"  {p['handle']}: {p['score']}/70 (structure={p['axes'].get('structural_compliance', 0)}, length={p['axes'].get('length_targets', 0)}, hype={p['axes'].get('anti_hype', 0)}, coverage={p['axes'].get('coverage', 0)})" for p in weak_products[:5])}
